...
用户订阅的 WTN 流统计信息以及网络状况，统计周期为 2s。
...
- **成员**
  ...
  | 名称 | 类型 | 描述 |
  | :-- | :-- | :-- |
  | publicStreamId | <code>string</code> |  WTN 流 ID。 |
  | RemoteStreamStats | <code>[RemoteStreamStats](#remotestreamstats)</code> | 远端媒体流统计信息 |


## LocalStatsException <span id="localstatsexception"></span>

类型: `interface`

本地用户媒体流异常信息

<p style="font-size: 16px;font-weight: bolder;"> exception <span id="localstatsexception-exception"></span></p> 

类型: `string`

本地媒体流异常信息。可取值参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常信息`列。

<p style="font-size: 16px;font-weight: bolder;"> code <span id="localstatsexception-code"></span></p> 

类型: <code>number</code>

本地媒体流异常代码。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常代码`列。

<p style="font-size: 16px;font-weight: bolder;"> isScreen <span id="localstatsexception-isscreen"></span></p> 

类型: <code>boolean</code>

本地媒体流是否为屏幕流：true-屏幕流，false-主流


## RemoteStatsException <span id="remotestatsexception"></span>

类型: `interface`

远端用户媒体流异常信息

<p style="font-size: 16px;font-weight: bolder;"> userId <span id="remotestatsexception-userid"></span></p> 

类型: <code>string</code>

发生异常的远端用户 ID

<p style="font-size: 16px;font-weight: bolder;"> exception <span id="remotestatsexception-exception"></span></p> 

类型: `string`

远端媒体流异常信息。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常信息`列。

<p style="font-size: 16px;font-weight: bolder;"> code <span id="remotestatsexception-code"></span></p> 

类型: <code>number</code>

远端媒体流异常代码。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常代码`列。

<p style="font-size: 16px;font-weight: bolder;"> isScreen <span id="remotestatsexception-isscreen"></span></p> 

类型: <code>boolean</code>

远端媒体流是否为屏幕流：true-屏幕流，false-主流
...
## PlayerVisibility <span id="playervisibility"></span>

类型: `interface`

播放器可见性状态及其相关信息（例如用户ID、媒体类型、流索引等）。

<p style="font-size: 16px;font-weight: bolder;"> descriptor <span id="playervisibility-descriptor"></span></p> 

类型: <code>{ userId?: string | undefined; mediaType: MediaType.VIDEO; streamIndex?: [StreamIndex](#streamindex) | undefined; streamId?: string | undefined; playerId?: string | undefined; }</code>

播放器描述信息，包含用户ID、媒体类型、流索引等标识信息。

 - **成员**

    | 名称 | 类型 | 描述 |
    | :-- | :-- | :-- |
    | userId | `string | undefined` | 用户ID，标识流的发布者。 |
    | mediaType | `MediaType.VIDEO` | 媒体类型，当前仅支持视频类型。 |
    | streamIndex | `StreamIndex | undefined` | 流索引，用于区分主流和屏幕共享流。 |
    | streamId | `string | undefined` | 流ID，唯一标识一个媒体流。 |
    | playerId | `string | undefined` | 播放器ID，唯一标识一个播放器实例。 |

<p style="font-size: 16px;font-weight: bolder;"> status <span id="playervisibility-status"></span></p> 

类型: <code>[VisibilityStatus](#visibilitystatus)[]</code>

播放器可见性状态数组，包含播放器当前的所有可见性状态。


## VisibilityStatus <span id="visibilitystatus"></span>
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | PLAYER_VISIBLE | `'visible'` | 播放器可见。 |
  | PLAYER_INVISIBLE_BY_UNMOUNT | `'unmount'` | 播放器未被挂载到 DOM，导致不可见。 |
  | PLAYER_INVISIBLE_BY_SIZE | `'size'` | 播放器因尺寸问题导致不可见（比如尺寸过小）。 |
  | PLAYER_INVISIBLE_BY_STYLE | `'style'` | 播放器因样式问题不可见。例如：`display: none`、`opacity: 0` 等。 |
  | PLAYER_INVISIBLE_BY_POSITION | `'position'` | 播放器因位置问题不可见（比如位置超出了可视区域）。 |
  | PLAYER_INVISIBLE_BY_COVERED | `'covered'` | 播放器因被覆盖而不可见。 |
...
<p style="font-size: 16px;font-weight: bolder;"> roomMode <span id="roomconfig-roommode"></span></p> 

类型: <code>[RoomMode](#roommode) | undefined</code>

房间的创建模式。支持创建同时支持使用 RTC 和 RTS 功能的房间（默认），或创建仅支持 RTS 功能的房间。
...
## RoomMode <span id="roommode"></span>
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | ROOM_MODE_RTC | `0` | 默认模式。房间内可同时使用 RTC 和 RTS 功能。 |
  | ROOM_MODE_RTS_ONLY | `1` | 房间内仅可使用 RTS 功能。该模式下，不可调用 RTC 相关的接口。<br>如果你只需要 RTS 功能，推荐通过集成 [RTS SDK](1162275) 创建应用。目前已支持 Android、iOS 以及 Web 端。 |
...
<p style="font-size: 16px;font-weight: bolder;"> preferCodecName <span id="videoencoderconfig-prefercodecname"></span></p> 

类型: <code>[VideoCodecType](#videocodectype) | undefined</code>

设置编码优先级。默认为 `'auto'`，无偏好，SDK 会根据当前视频流的分辨率、设备性能自动匹配适合的编解码格式。
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | AUTO | `'auto'` | 不指定编解码格式。SDK 会根据当前视频流的分辨率、设备性能自动匹配适合的编解码格式。 |
  | H264 | `'h264'` | H.264 |
  | VP8 | `'vp8'` | VP8 |
  | H265 | `'h265'` | H.265 <br>该格式对浏览器和分辨率有限制，具体查看 [Web SDK 浏览器兼容性和已知问题](111854#h265)。|
...
视频参数
 ...
 - **成员**
    ...
    | 名称 | 类型 | 描述 |
    | :-- | :-- | :-- |
    | width | `number | undefined` | 宽度，单位：px。<br>仅支持设为偶数，如果设为奇数，将导致 WTN 流发布失败。 |
    | height | `number | undefined` | 高度，单位：px。<br>仅支持设为偶数，如果设为奇数，云端媒体处理会直接拒绝，导致 WTN 流失败。 |
...
- **成员**
  ...
  | mediaType | <code>[PublicStreamType](#publicstreamtype) \| undefined</code> | WTN 流输出内容类型 |
  | sourceCrop | <code>{ x: number; y: number; w: number; h: number; } \| undefined</code> | WTN 流视频裁剪配置 |
...
- **成员**
  ...
  | VIDEO_STREAM_LOW | `1` | 下行网络不佳时，对视频流做降级处理。<br>该设置仅对发布端开启 Simulcast 功能时生效。参见[使用 Simulcast 功能](https://www.volcengine.com/docs/6348/1359023)。 |
...
## onWTNPushStateChangedEvent <span id="onwtnpushstatechangedevent"></span>

类型: `interface`

WTN 流发布结果。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnpushstatechangedevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> oldState <span id="onwtnpushstatechangedevent-oldstate"></span></p> 

类型: <code>[WTNPushState](#wtnpushstate)</code>

WTN 流发布任务原状态。

<p style="font-size: 16px;font-weight: bolder;"> newState <span id="onwtnpushstatechangedevent-newstate"></span></p> 

类型: <code>[WTNPushState](#wtnpushstate)</code>

WTN 流发布任务新状态。

<p style="font-size: 16px;font-weight: bolder;"> reason <span id="onwtnpushstatechangedevent-reason"></span></p> 

类型: <code>[WTNPushStateChangedReason](#wtnpushstatechangedreason)</code>

WTN 流发布任务状态变更原因。

<p style="font-size: 16px;font-weight: bolder;"> elapse <span id="onwtnpushstatechangedevent-elapse"></span></p> 
...
## WTNPushState <span id="wtnpushstate"></span>
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | INIT | `0` | 初始，调用 startPushWtn 后，状态由 init 变为 start。 |
  | START | `1` | 开始。 |
  | SUCCESS | `2` | 成功。 |
  | STOP | `3` | 停止。 |
  | FAIL | `4` | 推流失败。 |
...
## WTNPushStateChangedReason <span id="wtnpushstatechangedreason"></span>
...
- **成员**
  ...
  | :-- | :-- | :-- |
  | PUSH_SUCCESS | `0` | 成功。 |
  | START_PUSH | `1` | 主动调用 [startPushWTN](104478#wtnstream-startpushwtn)。 |
  | STOP_PUSH | `2` | 主动调用 [stopPushWTN](104478#wtnstream-stoppushwtn)。 |
  | IN_RETRY | `3` | 网络错误重试 |
  | RETRY_FAIL | `4` | 重试失败，重试三次后报此错误。 |
  | NO_PUSH_PERMISSION | `5` | token 无发布权限 |
  | STREAM_PUSH_BY_OTHER | `6` | 相同 stream_id 流被挤占 |


## onWTNPlayStateChangedEvent <span id="onwtnplaystatechangedevent"></span>

类型: `interface`

播放 WTN 流状态。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnplaystatechangedevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> oldState <span id="onwtnplaystatechangedevent-oldstate"></span></p> 

类型: <code>[WTNPlayState](#wtnplaystate)</code>

播放 WTN 流原状态。

<p style="font-size: 16px;font-weight: bolder;"> newState <span id="onwtnplaystatechangedevent-newstate"></span></p> 

类型: <code>[WTNPlayState](#wtnplaystate)</code>

播放 WTN 流新状态。

<p style="font-size: 16px;font-weight: bolder;"> reason <span id="onwtnplaystatechangedevent-reason"></span></p> 

类型: <code>[WTNPlayStateChangedReason](#wtnplaystatechangedreason)</code>

状态变更原因。

<p style="font-size: 16px;font-weight: bolder;"> elapse <span id="onwtnplaystatechangedevent-elapse"></span></p> 
...
## WTNPlayState <span id="wtnplaystate"></span>
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | INIT | `0` | 初始，调用 startPlayWtn 后，状态由 INIT 变为 START。 |
  | START | `1` | 开始。 |
  | SUCCESS | `2` | 成功。 |
  | STOP | `3` | 停止。 |
  | FAIL | `4` | 推流失败。 |
...
## WTNPlayStateChangedReason <span id="wtnplaystatechangedreason"></span>
...
- **成员**
  ...
  | :-- | :-- | :-- |
  | PLAY_SUCCESS | `0` | 成功。 |
  | START_PLAY | `1` | 主动调用 startPlayWtn。 |
  | STOP_PLAY | `2` | 主动调用 stopPlayWtn。 |
  | REMOTE_STOP | `3` | 发布端主动停止。 |
  | REMOTE_FAILURE | `4` | 发布端主动停止。 |
  | STREAM_BANNED | `5` | 流被封禁。 |
  | NO_PLAY_PERMISSION | `6` | token 无订阅权限。 |
  | STREAM_NOT_EXIST | `7` | 远端流不存在。 |
  | IN_RETRY | `8` | 网络错误重试 |
  | RETRY_FAIL | `9` | 重试失败，重试三次后报此错误。 |
  | INTERNAL | `10` | 内部原因（目前包含服务端三个上限错误） |
  | OVER_CLIENT_SUBSCRIBE_STREAM_LIMIT | `1310` | 超出单端订阅上限，一个引擎实例最多拉5路流 |
  | OVER_STREAM_SUBSCRIBE_USER_LIMIT | `1311` | 超出单流订阅人数上限，当前未配置限制 |
  | OVER_STREAM_SUBSCRIBE_REQUES_TLIMIT | `1312` | 超出单流订阅请求 QPS 上限，当前未配置限制 |


## onWTNRemoteAudioStateChangedEvent <span id="onwtnremoteaudiostatechangedevent"></span>

类型: `interface`

远端 WTN 流音频状态改变。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnremoteaudiostatechangedevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> state <span id="onwtnremoteaudiostatechangedevent-state"></span></p> 

类型: <code>[WTNRemoteAudioState](#wtnremoteaudiostate)</code>

WTN 流音频状态改变。

<p style="font-size: 16px;font-weight: bolder;"> reason <span id="onwtnremoteaudiostatechangedevent-reason"></span></p> 

类型: <code>[WTNRemoteAudioStateChangeReason](#wtnremoteaudiostatechangereason)</code>

远端 WTN 流音频状态变化原因


## WTNRemoteAudioState <span id="wtnremoteaudiostate"></span>
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | STOPED | `0` | 初始状态。在 ReasonMute 和 ReasonRemotestop 情况下，报告该状态。 |
  | STARTING | `1` | 本地用户已接收远端视频首包 |
  | DECODING | `2` | 远端视频流正在解码，正常播放 |
  | FROZEN | `3` | 远端视频流卡顿 |
  | FAILED | `4` | 拉流发生error，reason=internal，不对外 |


## WTNRemoteAudioStateChangeReason <span id="wtnremoteaudiostatechangereason"></span>
...
- **成员**
  ...
  | UNMUTE | `3` | 本地订阅远端流。 |
  | MUTE | `4` | 本地取消订阅远端流。 |
  | REMOTE_START | `5` | 远端流发布。 |
  | REMOTE_STOP | `6` | 远端流停止发布。 |


## onWTNRemoteVideoStateChangedEvent <span id="onwtnremotevideostatechangedevent"></span>

类型: `interface`

WTN 流视频状态改变。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnremotevideostatechangedevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> state <span id="onwtnremotevideostatechangedevent-state"></span></p> 

类型: <code>[WTNRemoteVideoState](#wtnremotevideostate)</code>

WTN 流视频状态。

<p style="font-size: 16px;font-weight: bolder;"> reason <span id="onwtnremotevideostatechangedevent-reason"></span></p> 

类型: <code>[WTNRemoteVideoStateChangeReason](#wtnremotevideostatechangereason)</code>

WTN 流视频状态改变原因。


## WTNRemoteVideoState <span id="wtnremotevideostate"></span>
...
## WTNRemoteVideoStateChangeReason <span id="wtnremotevideostatechangereason"></span>
...
## onWTNRemoteVideoStatsEvent <span id="onwtnremotevideostatsevent"></span>

类型: `interface`

WTN 流的视频统计信息。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnremotevideostatsevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> videoStats <span id="onwtnremotevideostatsevent-videostats"></span></p> 
...
## onWTNRemoteAudioStatsEvent <span id="onwtnremoteaudiostatsevent"></span>

类型: `interface`

WTN 流的音频统计信息。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnremoteaudiostatsevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> audioStats <span id="onwtnremoteaudiostatsevent-audiostats"></span></p> 
...
## onWTNFirstRemoteVideoFrameDecodedEvent <span id="onwtnfirstremotevideoframedecodedevent"></span>

类型: `interface`

WTN 流视频首帧事件。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnfirstremotevideoframedecodedevent-streamid"></span></p> 
...
## onWTNSEIMessageReceivedEvent <span id="onwtnseimessagereceivedevent"></span>

类型: `interface`

WTN 流包含的 SEI 消息。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnseimessagereceivedevent-streamid"></span></p> 

类型: <code>string</code>

WTN 流 ID。

<p style="font-size: 16px;font-weight: bolder;"> sei <span id="onwtnseimessagereceivedevent-sei"></span></p> 
...
## onWTNLocalStatsExceptionEvent <span id="onwtnlocalstatsexceptionevent"></span>

类型: `interface`

本地 WTN 媒体流异常事件。

<p style="font-size: 16px;font-weight: bolder;"> exception <span id="onwtnlocalstatsexceptionevent-exception"></span></p> 

类型: `string`

本地 WTN 媒体流异常信息。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常信息`列。

<p style="font-size: 16px;font-weight: bolder;"> code <span id="onwtnlocalstatsexceptionevent-code"></span></p> 

类型: <code>number</code>

本地 WTN 媒体流异常代码。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常代码`列。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnlocalstatsexceptionevent-streamid"></span></p> 
...
## onWTNRemoteStatsExceptionEvent <span id="onwtnremotestatsexceptionevent"></span>

类型: `interface`

远端 WTN 媒体流异常事件。

<p style="font-size: 16px;font-weight: bolder;"> exception <span id="onwtnremotestatsexceptionevent-exception"></span></p> 

类型: `string`

远端 WTN 媒体流异常信息。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常信息`列。

<p style="font-size: 16px;font-weight: bolder;"> code <span id="onwtnremotestatsexceptionevent-code"></span></p> 

类型: <code>number</code>

远端 WTN 媒体流异常代码。可取值，参见 [RTCExceptionCodeMap](#rtcexceptioncodemap) 中的`异常代码`列。

<p style="font-size: 16px;font-weight: bolder;"> streamId <span id="onwtnremotestatsexceptionevent-streamid"></span></p> 

类型: <code>string</code>

远端 WTN 媒体流 ID。

## RTCExceptionCodeMap<span id="rtcexceptioncodemap"></span>

| 异常信息 | 异常代码 | 说明 |
| --- | --- | --- |
| FRAMERATE_INPUT_TOO_LOW | 1001 | 采集视频帧率过低 |
| FRAMERATE_SENT_TOO_LOW | 1002 | 发送视频帧率过低 |
| SEND_VIDEO_BITRATE_TOO_LOW | 1003 | 发送视频码率过低 |
| RECV_VIDEO_DECODE_FAILED | 1005 | 接收视频码率过低 |
| AUDIO_INPUT_LEVEL_TOO_LOW | 2001 | 输入（采集）音量过低 |
| AUDIO_OUTPUT_LEVEL_TOO_LOW | 2002 | 输出（播放）音频过低 |
| SEND_AUDIO_BITRATE_TOO_LOW | 2003 | 发送音频码率过低 |
| FRAMERATE_INPUT_TOO_LOW_RECOVER | 3001 | 采集视频帧率过低恢复 |
| FRAMERATE_SENT_TOO_LOW_RECOVER | 3002 | 发送视频帧率过低恢复 |
| SEND_VIDEO_BITRATE_TOO_LOW_RECOVER | 3003 | 发送视频码率过低恢复 |
| RECV_VIDEO_DECODE_FAILED_RECOVER | 3005 | 接收视频码率过低恢复 |
| AUDIO_INPUT_LEVEL_TOO_LOW_RECOVER | 4001 | 输入（采集）音量过低恢复 |
| AUDIO_OUTPUT_LEVEL_TOO_LOW_RECOVER | 4002 | 输出（播放）音频过低恢复 |
| SEND_AUDIO_BITRATE_TOO_LOW_RECOVER | 4003 | 发送音频码率过低恢复 |